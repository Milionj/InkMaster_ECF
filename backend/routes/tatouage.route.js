...
// GET - Liste tous les tatouages
router.get('/', async (req, res) => {
  try {
    const [rows] = await db.execute('SELECT * FROM tatouage');
    res.json(rows);
  } catch (err) {
    console.error(err);
...
// GET - Récupère un tatouage par ID
router.get('/:id', async (req, res) => {
  try {
    const [rows] = await db.execute('SELECT * FROM tatouage WHERE id_tatouage = ?', [req.params.id]);
